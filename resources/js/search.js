document.addEventListener('DOMContentLoaded', function() {
    const searchInput = document.querySelector('input[placeholder*="Cari judul buku"]');
    if (!searchInput) return;

    let debounceTimer;
    const dropdown = document.createElement('div');
    dropdown.className = 'absolute z-50 mt-2 w-full bg-white rounded-lg shadow-lg border border-gray-200 hidden';
    ...
    function performSearch() {
        ...
        // Show loading state
        dropdown.innerHTML = '<div class="px-4 py-2 text-gray-500">Mencari...</div>';
        ...
        fetch(`/api/search/suggestions?q=${encodeURIComponent(query)}`)
            ...
            .catch(error => {
                ...
                dropdown.innerHTML = '<div class="px-4 py-2 text-red-500">Terjadi kesalahan</div>';
            });
    }

    function renderSuggestions(suggestions) {
        if (suggestions.length === 0) {
            dropdown.innerHTML = '<div class="px-4 py-2 text-gray-500">Buku tidak ditemukan</div>';
            return;
        }
